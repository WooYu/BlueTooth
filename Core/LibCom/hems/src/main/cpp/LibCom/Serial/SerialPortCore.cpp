...
SerialPortCore::SerialPortCore(const char *path) {
    SerialPortCore::path = path;
}

speed_t SerialPortCore::getBaudrate(int baudrate) {
    switch (baudrate) {
        case 0:
            return B0;
        case 50:
...
int SerialPortCore::setSpeed(int fd, int speed) {
    speed_t b_speed;
    struct termios cfg;
...
int SerialPortCore::setParity(int fd, int databits, int stopbits, char parity) {
...
int SerialPortCore::openSerialPort(SerialPortConfig config) {
    LOGD("Open device!");
    isClose = false;
    fd = open(path, O_RDWR);
    if (fd < 0) {
...
int SerialPortCore::readData(BYTE *data, int size,long timeout) {
    ...
    if (FD_ISSET(fd, &rfds)) {
        FD_ZERO(&rfds);
        FD_SET(fd, &rfds);
        retval = select(fd + 1, &rfds, NULL, NULL, m_timeval);
        if (retval == -1) {
            LOGE("Select error!");
        } else if (retval) {
    ...
    if (isClose) close(fd);
    LOGE("Read data ret:%d",ret);
    return ret;
}

int SerialPortCore::writeData(BYTE *data, int len) {
    int result;
    result = static_cast<int>(write(fd, data, static_cast<size_t>(len)));
    LOGE("send data result:%d",result);
    return result;
}

int SerialPortCore::setMode(int mode){
   struct termios options;
   if(tcgetattr(fd,&options) != 0){
       LOGE("The method tcgetattr exception!");
       return FALSE;
...
int SerialPortCore::clearFlush(int type) {
   return tcflush(fd, type);
}

void SerialPortCore::closePort() {
    LOGD("Close device!");
    isClose = true;
}